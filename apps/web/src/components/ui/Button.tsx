...
export function Button({ variant = "primary", className = "", disabled, children, ...props }: ButtonProps) {
  // ring-offset-color has no dark-mode-aware default (Tailwind hardcodes
  // white), which left a white halo around a focused button on the dark
  // palette — pointed at --color-bg instead so the offset always matches
  // whatever's actually behind the button.
  const base =
    "inline-flex items-center justify-center rounded-[10px] px-4 py-2.5 text-sm font-semibold transition-opacity focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--color-bg)] disabled:opacity-50 disabled:cursor-not-allowed";
